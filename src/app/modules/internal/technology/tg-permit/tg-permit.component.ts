...
@Component({
  selector: 'app-tg-permit',
  templateUrl: './tg-permit.component.html',
  styleUrls: ['./tg-permit.component.scss']
})
export class TgPermitComponent implements OnInit {
  ...
  syModuleData(response: any) {
    ...
    const params = {
      ...
      whereOperator: `=,=`,
      whereEqual: `permit,${this.syPrefixId}`,
    };
  ...
  resultData(fieldDeleted: string) {
    ...
    const params = {
      table: this.tableComponent,
      column: '*',
      whereCond: `WHERE${whereCd}`,
      whereField: `${this.tableSysEliminate}${whereFd}`,
      whereOperator: `=${whereOp}`,
      whereEqual: `${fieldDeleted}${whereEq}`,
    };
    this.serviceApi.innerAlias(params).subscribe({
      next: (response: any) => {
        console.log(response);
